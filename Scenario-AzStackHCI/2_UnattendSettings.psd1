...
@{
    VM0 = @{                                            # VM0 is the first VM - do not change this name
        ...
        Owner         = 'myhci'                         
        Timezone      = 'Iran Standard Time'       # ??? Timezone in OS you can do (PShell) to find yours  "get-timezone -ListAvailable | out-gridview -OutputMode Single | Select-Object ID"
        ...
        IPAddress     = "192.168.100.10"                   # (optional) static IP address to be assigned to the first NIC (alphabetical order in 1_VMs.psd1) - if not set, the IP address will be assigned by DHCP
    ...
    VM1 = @{                                            # VM1 is the second VM - do not change this name
        ...
        SystemLocale = 'en-US'
        UserLocale   = 'en-US'
        IPAddress     = "192.168.100.20"                 # (optional) static IP address to be assigned to the first NIC (alphabetical order in 1_VMs.psd1) - if not set, the IP address will be assigned by DHCP
        IPMask        = "24"                            # (required when IPAddress is used) subnet mask for the static IP address
        IPGateway     = "192.168.100.10"                    # (required when IPAddress is used) default gateway for the static IP address
        DNSIP         = "192.168.100.10"                 # (required when IPAddress is used) DNS server for the static IP address
    }
    VM2 = @{                                            
